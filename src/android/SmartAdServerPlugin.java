...
public class SmartAdServerPlugin extends GenericAdPlugin {
	...
	private String mInterstitialTarget;
	...
	@Override
	protected View __createAdView(String data) {
		String[] dataItems = data.split("[?]");
		String adId = dataItems[0];

		mBannerTarget = "";

		if(dataItems.length >= 2) {
			mBannerTarget = dataItems[1];
		}
	...
	protected void __loadAdView(View view) {
		if(view instanceof SASBannerView) {
			...
			ad.loadAd(siteId, mBannerPageId, mBannerFormatId, true, mBannerTarget, new SASAdView.AdResponseHandler() {
	...
	@Override
	protected Object __createInterstitial(String data) {
		String[] dataItems = data.split("[?]");
		String adId = dataItems[0];
		...
		if(dataItems.length >= 2) {
			mInterstitialTarget = dataItems[1];
		}
	...
	protected void __loadInterstitial(Object interstitial) {
		if(interstitial instanceof SASInterstitialView) {
			...
			// instantiate the response handler used when loading an ad on the banner
			ad.loadAd(siteId, mInterstitialPageId, mInterstitialFormatId, true, mInterstitialTarget, new SASAdView.AdResponseHandler() {
